"""C++ compiler facade — Local g++ → Piston → Judge0 fallback.
...
JUDGE0: https://rapidapi.com/judge0-official/api/judge0-ce → set JUDGE0_API_KEY env var
"""
...
class CppCompiler:
    ...
    def compile_and_run(self, code: str, stdin: str = "", timeout: int = DEFAULT_TIMEOUT) -> Dict[str, Any]:
    ...
    def _engine_order(self) -> List[str]:
        base = [ENGINE_LOCAL, ENGINE_PISTON, ENGINE_JUDGE0] if self.prefer_local else [
            ENGINE_JUDGE0, ENGINE_PISTON, ENGINE_LOCAL,
        ]
        return [
            e for e in base
    ...
    def _judge0(self, code: str, stdin: str, timeout: int) -> Optional[CompileResult]:
        ...
        for _ in range(15):
            time.sleep(1)
            poll = requests.get(
                f"{JUDGE0_URL}/{token}", headers=headers,
                params={"base64_encoded": "true"}, timeout=10,
            )
    ...
    def _piston(self, code: str, stdin: str, timeout: int) -> Optional[CompileResult]:
        ...
        return CompileResult(
            stdout=stdout, stderr=stderr, compile_error=compile_error,
            exit_code=exit_code, engine_used=ENGINE_PISTON,
    ...
    def _local(self, code: str, stdin: str, timeout: int) -> Optional[CompileResult]:
        ...
        try:
            ...
            try:
                run = subprocess.run(
                    [out], input=stdin, capture_output=True,
...
def grade_submission(
    ...
    for tc in test_cases:
        ...
        if run.get("compile_error"):
            ...
            results.append({
                "input": stdin, "expected": expected, "got": "",
                "points": points, "awarded": 0, "status": "compile_error",
                "stderr": run["compile_error"], "time_ms": run.get("time_ms", 0),
            })
            break
        ...
        results.append({
            ...
            "points": points, "awarded": points if passed else 0,
            "status": "pass" if passed else "fail",
            "stderr": run.get("stderr", ""), "time_ms": run.get("time_ms", 0),
        })
    return {
        "results": results, "score": score, "max_score": max_score,
        "passed": sum(1 for r in results if r["status"] == "pass"),
        "total": len(test_cases), "compile_error": compile_error,
        "engine_used": engine_used,
        "all_passed": score == max_score and bool(test_cases) and not compile_error,
    }
